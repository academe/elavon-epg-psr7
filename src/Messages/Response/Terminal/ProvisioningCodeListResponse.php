...
/**
 * Provisioning Code List Response.
 *
 * Parses PSR-7 responses for provisioning code lists (GET /terminals/{id}/provisioning-codes).
 *
...
class ProvisioningCodeListResponse
{
    ...
    private readonly ?array $provisioningCodes;
    ...
    public function __construct(private readonly ResponseInterface $response)
    {
        ...
        if ($this->isSuccessful()) {
            ...
            $this->provisioningCodes = $data['items'];
            $this->nextPage = $data['next'];
            $this->firstPage = $data['first'];
            $this->error = null;
        } else {
            $this->provisioningCodes = null;
            $this->nextPage = null;
            $this->firstPage = null;
    ...
    /**
     * Creates a ProvisioningCodeListResponse from a PSR-7 response.
     *
     * @param ResponseInterface $response PSR-7 HTTP response
     * @return static
     */
    ...
    /**
     * Gets the provisioning codes from a successful response.
     *
     * @return array<ProvisioningCode>|null Array of provisioning codes on success, null on error
     */
    public function getProvisioningCodes(): ?array
    {
        return $this->provisioningCodes;
    }
    ...
    /**
     * Parses a successful response into a paginated list of provisioning codes.
     *
     * @return array{items: array<ProvisioningCode>, next: string|null, first: string|null}
    ...
    private function parseSuccessResponse(): array
    {
        ...
        foreach ($data['items'] as $index => $itemData) {
            ...
            $provisioningCodes[] = ProvisioningCode::fromData($itemData);
        }

        return [
            'items' => $provisioningCodes,
