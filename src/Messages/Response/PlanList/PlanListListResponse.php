...
/**
 * PlanList List Response.
 *
 * Parses PSR-7 responses for paginated plan list lists (GET /plan-lists).
 */
class PlanListListResponse
{
    use HandlesErrors;

    private readonly ?array $planLists;
    ...
    /**
     * Creates a PlanListListResponse from a PSR-7 response.
     *
     * @param ResponseInterface $response PSR-7 HTTP response
     * @return static
     */
    ...
    /**
     * Gets the plan lists from a successful response.
     *
     * @return array<PlanList>|null Array of plan lists on success, null on error
     */
    public function getPlanLists(): ?array
    {
    ...
    /**
     * Parses a successful response into a paginated list of plan lists.
     *
     * @return array{items: array<PlanList>, next: string|null, first: string|null}
    ...
    private function parseSuccessResponse(): array
    {
        ...
        foreach ($data['items'] as $index => $itemData) {
            ...
            $planLists[] = PlanList::fromData($itemData);
        }
